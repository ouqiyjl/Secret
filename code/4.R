...
library(igraph)
#############解雇四的分析代码
interaction_data <- read_xlsx("H:/PCD_database/结果三统计/结果三重要互作死亡状态.xlsx")
interaction_data <- interaction_data[,c(1,5)]
interaction_data$A <- str_split_fixed(interaction_data$interaction,"-",2)[,1]
interaction_data$B <- str_split_fixed(interaction_data$interaction,"-",2)[,2]
############################一对重要互作的癌症的代码-----------------
#############对癌症进行一个KEGG和通路网络构建
cancer_name <- "ACC"
...
A_death_name <- as.character(interaction_data[which(interaction_data$cancer==cancer_name),3])
B_death_name <- as.character(interaction_data[which(interaction_data$cancer==cancer_name),4])
...
############提取一下重要亚群的差异基因
DEG_new <- DEG_new$gene[DEG_new$cluster %in% 
                          as.character(interaction_data[which(interaction_data$cancer == cancer_name),
                                           c(3,4)])]
...
rm(cds)
############提取A B modules 的基因
module_number <- as.numeric(str_split_fixed(c(A_module,B_module)," ",2)[,2])
...
if(dim(gene_set_enrich_kegg)[1]>20){
  gene_set_enrich_kegg <- gene_set_enrich_kegg[1:20,]
}
#GO富集柱形图绘制
...
p3 <- KEGG_bar("gene_set_enrich_kegg")+scale_fill_distiller(palette = "Oranges",direction = 1)
...
#################提取细胞表达
new_data <- new_data[,group_info$cell_names]
exp <- as.matrix(new_data@assays$RNA@data)
  ...
  if(length(id3)!=0){
    for (j in 1:length(id3)) {
      ...
      if(x%in%rownames(exp) & y%in%rownames(exp)){
        if((table(exp[x,A_death_local]==exp[x,B_death_local])[["TRUE"]] == length(B_death_local))|
           (table(exp[y,A_death_local]==exp[y,B_death_local])[["TRUE"]] == length(B_death_local))){
          s1=0}else{
  ...
  if(length(id5)!=0){
    for (z in 1:length(id5)) {
      ...
      if(m%in%rownames(exp) & n%in%rownames(exp)){
        if((table(exp[m,A_death_local]==exp[m,B_death_local])[["TRUE"]] == length(B_death_local))|
           (table(exp[n,A_death_local]==exp[n,B_death_local])[["TRUE"]] == length(B_death_local))){
          s2=0}else{
...
cross <- cbind.data.frame(int_path,score2)

data_file <- paste0("H:/PCD_database/result4/",cancer_name,"/cross_raw.csv")
write.csv(cross,data_file)
#############进行cross的筛选和画图
crosstalk_1 <- cross
...
pdf(file = pdf_file,width = 11.67,height = 8.27)
