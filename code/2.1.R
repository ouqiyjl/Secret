...
new_data <- AddMetaData(new_data,metadata = Idents(new_data),col.name = "Death_state")

mean_score <- aggregate(new_data$malignant_Score1, by=list(type=new_data$Death_state),mean)
...
get_earliest_principal_node <- function(cds, Death_state){
  cell_ids <- which(colData(cds)[, "Death_state"] == Death_state)
...
annotation_col = data.frame(
  Death_state  =names(color_PCD))
rownames(annotation_col) = names(color_PCD)

ann_colors = list(
  Death_state =  c("Alkaliptosis"="#41AB5D","Anoikis" = "#A35D34","Autophagy"="#ABCADE",
                  "Cuproptosis"="#8DD3C7",
                  "Entotie_cell_death"="#B3DE69","Extrinsic_apoptosis"="#3B77AC",
                  "Ferroptosis"="#BEBADA",
                  "Immunogenic_cell_death"="#C6DBEF","Intrinsic_apoptosis"="#80B1D3",
